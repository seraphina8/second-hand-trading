...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.request.get("/productFineness/page", {
        params: data
      }).then(res => {
    ...
    submit() {
      ...
      if (this.formData.id === undefined) {
        this.request.post("/productFineness", this.formData).then(res => {
          if (res.code === 200) {
        ...
        //更新
        this.request.put("/productFineness", this.formData).then(res => {
          if (res.code === 200) {
            this.isDrawerDialog = false
            this.$message({
              message: res.msg,
    ...
    batchDelete(rows) {
      ...
      this.$confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
        ...
        //批量删除数据
        this.request.delete("/productFineness", {data: ids}).then(res => {
          if (res.code === 200) {
            this.$message({
              message: res.msg,
              type: 'success'
