...
public class SysRoleController {
    ...
    public Result<PageVO<SysRole>> findPage(@RequestParam Map<String, Object> query, @RequestParam(defaultValue = "1") Integer pageNum, @RequestParam(defaultValue = "10") Integer pageSize) {
        Page<SysRole> page = service.page(query, pageNum, pageSize);
        page.getRecords().forEach(sysRole -> {
            sysRole.selectMenuList(sysRoleMenuService);
        });
    ...
    @PostMapping()
    public Result add(@Validated @RequestBody SysRole entity) {
        service.save(entity);
    ...
    @PutMapping()
    public Result update(@Validated @RequestBody SysRole entity) {
    ...
    @GetMapping("/{id}")
    public Result<SysRole> getById(@PathVariable("id") Integer id) {
