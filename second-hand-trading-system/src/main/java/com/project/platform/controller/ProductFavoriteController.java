...
public class ProductFavoriteController {
    ...
    @GetMapping("/page")
    public Result<PageVO<ProductFavorite>> findPage(@RequestParam Map<String, Object> query,@RequestParam(defaultValue = "1") Integer pageNum,@RequestParam(defaultValue = "10") Integer pageSize) {
        Page<ProductFavorite> page = service.page(query, pageNum, pageSize);
        page.getRecords().forEach(item->{
            item.selectProduct(productService);
    ...
    @PostMapping()
    public Result add(@Validated @RequestBody ProductFavorite entity) {
        service.save(entity);
    ...
    @PutMapping()
    public Result update(@Validated @RequestBody ProductFavorite entity) {
    ...
    @GetMapping("/{id}")
    public Result<ProductFavorite> getById(@PathVariable("id") Integer id) {
        ProductFavorite entity = service.getById(id);
